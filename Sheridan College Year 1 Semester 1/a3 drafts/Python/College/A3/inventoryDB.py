...
def main(): 
    while True: #while user doesnt choose to quit
        ...
        if ch.isnumeric() == False:
            print(f"ERROR: {ch} is a invlaid input \nPlease enter a valid int ")# error msg
        if ch == '1':#show_all()
            show_all(database)#function to show all items 
            print('Michael Aaron Nolk. 991673010') #waterMark 
        elif ch == '2':#look_up()
...
def choice():
    ...
    10. Quit the proram
    """)#options
    #print(f"CATEGORY_LIST(DATBASE) = {CATEGORY_LIST} ") # DISPLAYING THE CATEGORY LIST CHANGES (UnComment to see the list)
