...
CATEGORY_LIST = ['fruit', 'dairy', 'vegetable']#creating a list full of my default categories

def main(): 
    databaseDB = database() #getting my dict from the text file 
    while True: #while user doesnt choose to quit
        ...
        if ch == '1':#show_all()
            show_all(databaseDB)#function to show all items 
        elif ch == '2':#look_up()
            look_up(databaseDB)#function that looks up all items 
            writeDB(databaseDB) #updating database
        elif ch == '3':
            add_item(databaseDB)
            writeDB(databaseDB) #updating database
        elif ch == '4': 
            change_item(databaseDB)#changing item in DB 
            writeDB(databaseDB) #updating database
        elif ch == '5': 
            delete_item(databaseDB)#deleting item from DB
            writeDB(databaseDB) #updating database 
        elif ch == '6': 
            find_item_category(databaseDB)#finding items given category 
            writeDB(databaseDB) #updating database
        elif ch == '7': 
            itemCount_priceCategory(databaseDB)#Item count, average price by category
            writeDB(databaseDB) #updating database
        elif ch == '8': 
            priceUP(databaseDB)#finding the most exspensive item 
            writeDB(databaseDB) #updating database
        elif ch == '9':
            total_price(databaseDB)#total price by item 
            writeDB(databaseDB) #updating database
        elif ch == '10':
...
def database(): #this is my funciton to read from the text file and convert it to a dict
    databaseDict = dict() #creating a empty dictionary 
    database = open("inventoryDB.txt", 'r') #opening the inventoryDB.txt file in read mode
    textDB_List = database.readlines() #reading all lines in text file and creating a list 
    '''
    print(textDB_List) #Uncomment if you would like to view the list
    '''
    for elements in textDB_List:
        ...
        databaseDict[elements[:3]] = lst #creating my dict with keys of the first 3 characters from each line in the text file 
    '''
    print(databaseDict)
    '''
    database.close() #closing text file 
    return databaseDict #returning my database dict

def writeDB(dictonary): #function to write my dict to text file 

    f = open('inventoryDB.txt', 'w')
    for keys, values in dictonary.items():
        for i in range(len(values)):
            values[i]= str(values[i]) #converting from num to str 
        values = ' '.join(values)# joining my list to get rid of the [] symbols and '' symbols
